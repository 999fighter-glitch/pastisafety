...
async function getEffectiveTelegramConfig() {
  ...
  // If missing, attempt to fetch from Firestore systemConfig via REST API
  if (!token || !chatId) {
    try {
...
async function startServer() {
  ...
  app.post("/api/telegram-config", async (req, res) => {
    ...
    // Persist to Firestore systemConfig/telegram via REST API
    try {
      const projectId = process.env.VITE_FIREBASE_PROJECT_ID || "sustained-sandbox-w5xj8";
      const dbId = process.env.VITE_FIREBASE_DATABASE_ID || "ai-studio-8f3dbb41-3cc2-4a80-9f09-efcd98f45929";
      const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${dbId}/documents/systemConfig/telegram`;
    ...
    res.json({
      success: true,
      message: "Telegram configuration updated on server and saved to Firebase Firestore database."
    });
  });

  // GET Firebase Backend Details
  ...
  app.get("/api/test-telegram", async (req, res) => {
    ...
    if (!token) {
      return res.status(400).json({ error: "Telegram token missing. Sila tetapkan TELEGRAM_BOT_TOKEN dalam tetapan atau fail .env" });
    }
  ...
  app.post("/api/send-telegram", async (req, res) => {
    ...
    if (!token || !chatId) {
      return res.status(400).json({ error: "Telegram configuration missing. Sila tetapkan TELEGRAM_BOT_TOKEN dan TELEGRAM_CHAT_ID dalam fail .env atau tetapan bot." });
    }
